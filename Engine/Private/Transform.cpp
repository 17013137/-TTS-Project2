...
HRESULT CTransform::Turn_Bind_OnGraphicDevice(const _float3& vAxis, _float fTimeDelta)
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
	...
	memcpy(&Matrix.m[0][0], &vRight, sizeof(_float3));
	memcpy(&Matrix.m[1][0], &vUp, sizeof(_float3));
	memcpy(&Matrix.m[2][0], &vLook, sizeof(_float3));
...
HRESULT CTransform::Go_X(_float fTimeDelta)
{
	_float3		vPosition = Get_State(CTransform::STATE_POSITION);
...
HRESULT CTransform::Go_Y(_float fTimeDelta)
{
	_float3		vPosition = Get_State(CTransform::STATE_POSITION);
...
HRESULT CTransform::Go_Z(_float fTimeDelta)
{
	_float3		vPosition = Get_State(CTransform::STATE_POSITION);
...
HRESULT CTransform::Look(const _float3 & vTargetPos)
{
	_float3		vScale = Get_Scale();
