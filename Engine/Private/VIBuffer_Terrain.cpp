...
HRESULT CVIBuffer_Terrain::NativeConstruct_Prototype(_uint iNumVerticesX, _uint iNumVerticesZ)
{
	...
	for (_uint i = 0; i < m_iNumVerticesZ; ++i)
	{
		for (_uint j = 0; j < m_iNumVerticesX; ++j)
		{
			...
			pVertices[iIndex].vTexUV = _float2(j / (m_iNumVerticesX - 1.f) * 10.f, i / (m_iNumVerticesZ - 1.f) * 10.f);
...
//			pVertices[iIndex].vTexUV = _float2((j / (m_iNumVerticesX - 1.f) * 10.f)*5, (i / (m_iNumVerticesZ - 1.f) * 10.f)*5);
//		}
//	}
//
//	memcpy(m_pVertices, pVertices, sizeof(VTXTEX) * m_iNumVertices);
